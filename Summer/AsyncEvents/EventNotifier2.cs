...
public static class EventNotifier2
{
    private static readonly Dictionary<Type, List<EventHandlerWrapper>> EventListeners = new();

    public delegate Task AsyncEventHandler<in T>(object? sender, T msg) where T : IAsyncEvent;
    ...
    private static bool IsAValidAsyncEventHandler(MethodBase method)
    {
        ...
        return parameters.Length == 2 &&
               ...
               typeof(IAsyncEvent).IsAssignableFrom(parameters[1].ParameterType);
    }

    public static void Subscribe(Type eventType, EventHandlerWrapper handlerWrapper)
    {
        ...
        Console.WriteLine($"Adding event listener for {eventType.Name} - {handlerWrapper.InstanceType.Name}.{handlerWrapper.Method.Name}");
